...
exports.renderManpowerEnquiryAdminNotification = renderManpowerEnquiryAdminNotification;
const base_layout_1 = require("./base.layout");
function renderManpowerEnquiryAdminNotification(data) {
    ...
    const positionsHtml = data.positions
      ...
      <tr>
        <td style="padding: 10px 12px; border-bottom: 1px solid #E2E8F0; font-size: 13px; color: #1E293B;">
          <strong>#${idx + 1}. ${(0, base_layout_1.escapeHtml)(p.roleTitle)}</strong><br/>
          <span style="font-size: 11px; color: #64748B;">Category: ${(0, base_layout_1.escapeHtml)(p.categorySlug)}</span>
        </td>
    ...
    <div style="margin-bottom: 24px;">
      ...
      <h2 style="margin: 12px 0 4px 0; font-size: 18px; color: #0B192C; font-weight: 700;">
        Manpower Requirement from ${(0, base_layout_1.escapeHtml)(data.companyName)}
      </h2>
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom: 20px; border: 1px solid #E2E8F0; border-radius: 6px; overflow: hidden; background-color: #F8FAFC;">
      ...
      <tr>
        ...
        <td style="padding: 10px 14px; border-bottom: 1px solid #E2E8F0; font-size: 13px; color: #0B192C;">
          ${(0, base_layout_1.escapeHtml)(data.contactPerson)} ${data.contactDesignation ? `(${(0, base_layout_1.escapeHtml)(data.contactDesignation)})` : ''}
        </td>
      </tr>
      <tr>
        <td style="padding: 10px 14px; border-bottom: 1px solid #E2E8F0; font-size: 12px; font-weight: 600; color: #475569;">
        ...
        <td style="padding: 10px 14px; border-bottom: 1px solid #E2E8F0; font-size: 13px; color: #0B192C;">
          <a href="mailto:${(0, base_layout_1.escapeHtml)(data.email)}" style="color: #1E40AF; text-decoration: none;">${(0, base_layout_1.escapeHtml)(data.email)}</a> | ${(0, base_layout_1.escapeHtml)(data.phone)}
        </td>
      </tr>
      <tr>
        <td style="padding: 10px 14px; border-bottom: 1px solid #E2E8F0; font-size: 12px; font-weight: 600; color: #475569;">
        ...
        <td style="padding: 10px 14px; border-bottom: 1px solid #E2E8F0; font-size: 13px; color: #0B192C;">
          ${(0, base_layout_1.escapeHtml)(data.city)}${data.deploymentLocation ? ` (Deployment: ${(0, base_layout_1.escapeHtml)(data.deploymentLocation)})` : ''}
        </td>
      </tr>
      <tr>
        ...
        <td style="padding: 10px 14px; font-size: 13px; color: #0B192C;">
          ${(0, base_layout_1.escapeHtml)(data.preferredTimeline || 'Standard timeline')}
        </td>
      </tr>
    </table>
    ...
    ${data.specialRequirements
      ...
      <div style="margin-bottom: 20px; padding: 12px 14px; background-color: #F8FAFC; border-left: 3px solid #3B82F6; border-radius: 4px;">
        ...
        <p style="margin: 4px 0 0 0; font-size: 13px; color: #1E293B; line-height: 1.5;">
          ${(0, base_layout_1.escapeHtml)(data.specialRequirements)}
        </p>
      </div>
    `
        : ''}
...
ROLES BREAKDOWN:
${data.positions
        .map((p, i) => `#${i + 1} ${p.roleTitle} (Category: ${p.categorySlug}) — Headcount: ${p.headcount}`)
        .join('\n')}
