...
exports.manpowerEnquiryService = exports.ManpowerEnquiryService = void 0;
...
const employer_repository_1 = require("../repositories/employer.repository");
const manpower_enquiry_repository_1 = require("../repositories/manpower-enquiry.repository");
const audit_log_repository_1 = require("../repositories/audit-log.repository");
const notification_service_1 = require("./notification.service");
class ManpowerEnquiryService {
    employerRepo;
    manpowerRepo;
    auditRepo;
    notification;
    static inFlightLocks = new Map();
    constructor(employerRepo = employer_repository_1.employerRepository, manpowerRepo = manpower_enquiry_repository_1.manpowerEnquiryRepository, auditRepo = audit_log_repository_1.auditLogRepository, notification = notification_service_1.notificationService) {
     ...
     */
    async submitEnquiry(input, context = {}) {
        const db = (0, connection_1.getDbClient)();
        // 1. Compute deterministic canonical request hash
        const requestHash = (0, manpower_enquiry_schema_1.computeCanonicalRequestHash)(input);
        ...
        if (idempotencyKey) {
            ...
            if (existingKeyEnquiry) {
                ...
                if (existingKeyEnquiry.request_hash === requestHash) {
                    ...
                    logger_1.logger.warn(`Idempotency key conflict: Key "${idempotencyKey}" previously used for differing requisition payload`, {
                        existingHash: existingKeyEnquiry.request_hash,
                        incomingHash: requestHash,
                    });
                    throw new app_error_1.AppError('Idempotency key conflict: This key was previously used for a different manpower requisition.', 409, 'IDEMPOTENCY_KEY_CONFLICT');
                }
            }
        }
        // 3. Concurrency Protection via In-Process Mutex & MariaDB Advisory Lock
        const lockKey = `clc_mp_lock_${crypto_1.default
            .createHash('sha256')
        ...
        try {
            ...
            const clientDialect = String(db.client?.dialect || '');
            const isMySQL = clientDialect.includes('mysql') || clientDialect.includes('mariadb');
            if (isMySQL) {
                try {
                    if (typeof db.client?.acquireConnection === 'function') {
                        dedicatedConn = await db.client.acquireConnection();
                    }
                }
                catch (connErr) {
                    logger_1.logger.warn('Failed to acquire dedicated connection for advisory lock; proceeding with in-process lock only', { error: connErr });
                    dedicatedConn = null;
                }
                if (dedicatedConn) {
                    try {
                        ...
                        if (!hasAdvisoryLock) {
                            logger_1.logger.warn(`Could not acquire MariaDB advisory lock for key "${lockKey}" within 5s timeout`);
                        }
                    }
                    catch {
            ...
            if (idempotencyKey) {
                ...
                const recentDuplicate = await this.manpowerRepo.findRecentDuplicate(input.companyName, input.email, requestHash, 15 * 60 * 1000);
                if (recentDuplicate) {
                    logger_1.logger.info(`Recent duplicate manpower enquiry detected for ${input.companyName} / ${input.email}; returning existing reference`);
                    return {
                        success: true,
            ...
            const manpowerEnquiryId = crypto_1.default.randomUUID();
            ...
            try {
                await (0, transaction_1.withTransaction)(async (trx) => {
                    ...
                    let employerId;
                    const existingEmployer = await this.employerRepo.findExistingEmployer(input.companyName, input.email, trx);
                    if (existingEmployer) {
                        ...
                        employerId = crypto_1.default.randomUUID();
                ...
                const isDuplicateKey = txErr?.code === 'ER_DUP_ENTRY' ||
                    txErr?.errno === 1062 ||
                ...
                if (isDuplicateKey && idempotencyKey) {
                    logger_1.logger.info(`Database unique constraint caught for idempotency key "${idempotencyKey}"; resolving conflict`);
            ...
            logger_1.logger.info(`Manpower enquiry created successfully: ${publicReference} for ${input.companyName}`);
            return {
                success: true,
            ...
            if (dedicatedConn) {
                try {
                    ...
                    logger_1.logger.error('Failed to release advisory lock for manpower enquiry on dedicated connection', releaseErr);
                }
                finally {
                    try {
                        await db.client.releaseConnection(dedicatedConn);
                    }
                    catch (poolErr) {
                        logger_1.logger.error('Failed to release dedicated connection back to pool', poolErr);
                    }
                }
            }
        }
    ...
    async getEnquiryById(id) {
        ...
        if (!detail) {
            throw new app_error_1.AppError('The requested manpower enquiry was not found.', 404, 'MANPOWER_ENQUIRY_NOT_FOUND');
        }
        return detail;
    }
    /**
     ...
     */
    async updateEnquiryStatus(id, input, actor) {
        ...
        const allowed = manpower_enquiry_schema_1.ALLOWED_MANPOWER_STATUS_TRANSITIONS[currentStatus] || [];
        if (!allowed.includes(nextStatus)) {
            throw new app_error_1.AppError(`Invalid status transition from "${currentStatus}" to "${nextStatus}". Allowed transitions: ${allowed.join(', ') || 'none (terminal state)'}`, 400, 'INVALID_STATUS_TRANSITION');
        }
     ...
     */
    async updateEnquiry(id, input, actor) {
        const current = await this.manpowerRepo.findById(id);
        if (!current) {
            throw new app_error_1.AppError('The requested manpower enquiry was not found.', 404, 'MANPOWER_ENQUIRY_NOT_FOUND');
        }
...
exports.ManpowerEnquiryService = ManpowerEnquiryService;
exports.manpowerEnquiryService = new ManpowerEnquiryService();
