...
export interface JobApplicationAdminNotificationData {
  reference: string;
  jobTitle: string;
  jobCategory: string;
...
export function renderJobApplicationAdminNotification(data: JobApplicationAdminNotificationData): {
  subject: string;
  html: string;
  text: string;
} {
  ...
  const contentHtml = `
    <div style="margin-bottom: 24px;">
      ...
      <h2 style="margin: 12px 0 4px 0; font-size: 18px; color: #0B192C; font-weight: 700;">
        Candidate Profile Submitted for ${escapeHtml(data.jobTitle)}
      </h2>
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom: 24px; border: 1px solid #E2E8F0; border-radius: 6px; overflow: hidden; background-color: #F8FAFC;">
      ...
      <tr>
        ...
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 14px; color: #1E293B;">
          <strong>${escapeHtml(data.jobTitle)}</strong> (${escapeHtml(data.jobCategory)})
        </td>
      </tr>
      <tr>
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; font-weight: 600; color: #475569;">
      ...
      <tr>
        ...
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 14px; color: #1E293B;">
          ...
          ${data.whatsapp ? `<br><span style="font-size: 12px; color: #64748B;">WhatsApp: ${escapeHtml(data.whatsapp)}</span>` : ''}
        </td>
      </tr>
      <tr>
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; font-weight: 600; color: #475569;">
        ...
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 14px; color: #1E293B;">
          ${escapeHtml(data.nationality)} • Current: ${escapeHtml(data.currentLocation)}
        </td>
      </tr>
      <tr>
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; font-weight: 600; color: #475569;">
    ...
    ${
      data.coverLetter
        ? `<div style="margin-bottom: 24px; padding: 16px; background-color: #FFFFFF; border: 1px solid #E2E8F0; border-radius: 6px;">
            ...
            <p style="margin: 0; font-size: 14px; line-height: 1.6; color: #1E293B; white-space: pre-wrap;">${escapeHtml(data.coverLetter)}</p>
          </div>`
        : ''
    }
