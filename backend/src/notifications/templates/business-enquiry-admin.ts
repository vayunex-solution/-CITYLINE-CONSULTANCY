...
export interface BusinessEnquiryAdminNotificationData {
  reference: string;
  fullName: string;
  email: string;
  phone: string;
...
export function renderBusinessEnquiryAdminNotification(data: BusinessEnquiryAdminNotificationData): {
  subject: string;
  html: string;
  text: string;
} {
  ...
  const contentHtml = `
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom: 24px; border: 1px solid #E2E8F0; border-radius: 6px; overflow: hidden; background-color: #F8FAFC;">
      ...
      <tr>
        ...
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 14px; color: #1E293B;">
          <a href="mailto:${escapeHtml(data.email)}" style="color: #0B192C; text-decoration: underline;">
      ...
      <tr>
        ...
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 14px; color: #1E293B;">
          ${escapeHtml(data.phone)} ${data.whatsapp ? `(WhatsApp: ${escapeHtml(data.whatsapp)})` : ''}
        </td>
      </tr>
      <tr>
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; font-weight: 600; color: #475569;">
        ...
        <td style="padding: 12px 16px; border-bottom: 1px solid #E2E8F0; font-size: 14px; color: #1E293B;">
          ${
            data.documentsCount > 0
              ? `<strong style="color: #10B981;">✓ ${data.documentsCount} document(s) attached to this email</strong>${
                  data.documentNames && data.documentNames.length > 0
                    ? ` <span style="font-size: 12px; color: #64748B;">(${escapeHtml(data.documentNames.join(', '))})</span>`
                    : ''
                }`
              : 'None provided'
          }
