...
export function renderJobApplicationConfirmation(data: JobApplicationConfirmationData): {
  subject: string;
  html: string;
  text: string;
} {
  ...
  const contentHtml = `
    <div style="margin-bottom: 24px;">
      ...
      <p style="margin: 0 0 16px 0; font-size: 15px; line-height: 1.6; color: #334155;">
        Thank you for submitting your candidate profile to <strong>Cityline Consultancy</strong>. Your application for the position of <strong>${escapeHtml(data.jobTitle)}</strong> (${escapeHtml(data.jobCategory)}) has been registered with our recruitment operations team.
      </p>
    </div>
    ...
    <table role="presentation" border="0" cellpadding="0" cellspacing="0" width="100%" style="margin-bottom: 24px; border: 1px solid #E2E8F0; border-radius: 6px; overflow: hidden; background-color: #FFFFFF;">
      ...
      <tr>
        ...
        <td style="padding: 10px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; font-weight: 600; color: #1E293B;">${escapeHtml(data.jobTitle)}</td>
      </tr>
      <tr>
        <td style="padding: 10px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; color: #64748B;">Trade Category</td>
        <td style="padding: 10px 16px; border-bottom: 1px solid #E2E8F0; font-size: 13px; color: #1E293B;">${escapeHtml(data.jobCategory)}</td>
      </tr>
      <tr>
